...
// ------------------- GIS Setup -------------------
function initGIS() {
  console.log("✅ GIS library loaded, initializing tokenClient...");

  tokenClient = google.accounts.oauth2.initTokenClient({
    client_id: CLIENT_ID,
    scope: SCOPES,
    callback: (tokenResponse) => handleNewToken(tokenResponse),
  });
    ...
    if (tokenClient) {
      ...
      setTimeout(() => {
        if (tokenClient) {
          ...
          console.error("❌ tokenClient still not available");
        }
      }, 1000);
    }
  });
