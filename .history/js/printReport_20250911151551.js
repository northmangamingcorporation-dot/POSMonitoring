function printReport(card) {
  ...
  // clone the card safely
  const clone = card.cloneNode(true);

  // remove all buttons/controls
  ...
  // collect images
  const images = Array.from(clone.querySelectorAll('img')).map(img => img.src);
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .images { display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:10px; margin-top:15px; }
          .images img { width:100%; height:180px; object-fit:cover; border:1px solid #ddd; border-radius:6px; }
