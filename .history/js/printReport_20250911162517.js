function printReport(card) {
  try {
    ...
    if (!cardEl) return console.error('printReport: card element not found');
    ...
    const location = (cardEl.querySelector('i.fa-map-marker-alt')?.parentElement?.textContent.replace('📍','').trim()) || '';
    const description = (cardEl.querySelector('.report-desc')?.textContent || '').trim();

    // Collect images > 80px (skip thumbnails)
    ...
    const photoGrid = images.slice(1);
...
<style>
body { font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial; padding:10mm; -webkit-print-color-adjust: exact; color:#111; font-size:11pt; line-height:1.4; }
.container { width:100%; margin:0 auto; }
h1 { font-size:18pt; margin:0; }
h2 { font-size:14pt; margin-bottom: 540px; color:#374151; }
.meta { margin-bottom:10px; }
.meta p { margin:4px 0; }
.section { margin:10px 0; page-break-inside: avoid; }
.photo-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(250px,1fr)); gap:8px; margin-top:10px; }
.photo-grid img { width:100%; height:250px; object-fit:cover; border-radius:6px; border:1px solid #e5e7eb; display:block; }
.cover { width:100%; height:220px; object-fit:cover; border-radius:6px; border:1px solid #e5e7eb; display:block; margin-bottom:12px; }
  ...
  <div class="container">
    ...
    <div class="section report-section">
      ...
      <div class="photo-grid">
        ${photoGrid.length ? photoGrid.map(src => `<img src="${src}">`).join('') : '<div style="color:#666">No additional photos</div>'}
      </div>
    </div>
...
function escapeHtml(s){if(s==null)return'';return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');}
    ...
    if ('srcdoc' in iframe) {
      ...
      iframe.onload = () => waitForIframeImagesAndPrint(iframe, 3000);
    } else {
    ...
    function waitForIframeImagesAndPrint(ifr, timeout = 3000) {
      try {
        ...
        const onLoaded = () => { if (done) return; loaded++; if (loaded === imgs.length) { done = true; triggerPrint(); } };
        imgs.forEach(img => { if (img.complete) onLoaded(); else { img.addEventListener('load', onLoaded, {once:true}); img.addEventListener('error', onLoaded, {once:true}); } });
        setTimeout(() => { if (!done) { done = true; triggerPrint(); } }, timeout);
      } catch (err) { console.error('printReport: error waiting images', err); triggerPrint(); }

      function triggerPrint() {
        try { const win = ifr.contentWindow; win.focus(); win.print(); } 
        catch (e) { console.error('printReport: print failed', e); alert('Unable to open print dialog.'); } 
        finally { setTimeout(()=>{ try{document.body.removeChild(iframe);}catch(e){} },500); }
      }
    }

    function escapeHtml(s){ if(s==null)return''; return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }

  } catch (err) { console.error('printReport error:', err); alert('Error preparing print.'); }
}
